...
class GalvoTuningScanGUIHelper(ScanGUIHelper):
    ...
    '''
    def __init__(self, name: str, flags: int, params: GalvoTuningScanParams, settings: Dict[str, Any], octui, log_level):
    ...
    def createEngineComponents(self, octuiparams: OCTUiParams, samples_per_record: int):
        ...
        ascan_endpoint = self._createAscanEndpoint(sfe, vshape, octuiparams.vtx, get_logger('galvo-ascan-endpoint-plot', self.log_level))
        ...
        spectra_endpoint = SpectraStackHostTensorEndpointUInt16(sfe_spectra, shape_spectra, get_logger('galvo-spectra-endpoint-plot', self.log_level))
        ...
        spectra_storage = SimpleStackUInt16(get_logger('galvo-spectra-storage', self.log_level))
        sfec = StackFormatExecutorConfig()
        sfe = StackFormatExecutor()
        sfe.initialize(sfec)
        storage_endpoint = SpectraStackEndpoint(sfe, spectra_storage, log=get_logger('galvo-spectra-endpoint', self.log_level))
